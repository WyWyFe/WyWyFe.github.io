...
First, run Command Prompt as an administrator. We will use **ncpa.cpl** to jump to the adapter in the network connections Control Panel applet. Right click on the adapter / interface you wish to configure and open the Properties option. We are interested in the IPv4 option here. Double click it and configure it as shown below. Click "yes" when it warns you about the IP addresses being the same - this is of little consequence. We will make sure this is valid ourselves. ![ipv4 config](/assets/img/image(15).png)
